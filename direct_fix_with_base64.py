#!/usr/bin/env python3
"""
Direct fix using base64 to avoid escaping issues
"""
...
print("=" * 80)
print("Direct Fix - MySQL + Frontend Panel")
print("=" * 80)
...
if '1' not in mysql_result:
    print("  MySQL failed - triggering OpenClaw auto-fix...")
    stdin, stdout, stderr = ssh.exec_command("bash /root/.openclaw/monitoring/scripts/enhanced_mysql_fix.sh 2>&1 | tail -5")
    print("  Waiting for MySQL fix (65 seconds)...")
    time.sleep(65)
    ...
    if '1' in stdout.read().decode():
        ...
        print("  MySQL still failed - exiting")
        sys.exit(1)
else:
    print("  MySQL OK")

# Step 2: Create fix panel HTML using base64
print("\n[2/6] Creating fix panel template...")
...
# Create directory and write file via base64
ssh.exec_command(f"mkdir -p {SERVER_PATH}/templates/includes", timeout=5)

write_cmd = f'echo "{encoded}" | base64 -d > {SERVER_PATH}/templates/includes/fix_panel.html'
stdin, stdout, stderr = ssh.exec_command(write_cmd, timeout=10)
time.sleep(2)

# Verify file exists
stdin, stdout, stderr = ssh.exec_command(f"test -f {SERVER_PATH}/templates/includes/fix_panel.html && echo 'EXISTS' || echo 'MISSING'")
if 'EXISTS' in stdout.read().decode():
    print("  Fix panel template CREATED")
else:
    print("  ERROR: Template not created")

# Step 3: Add API views
print("\n[3/6] Adding API views...")
...
# Encode to base64
api_encoded = base64.b64encode(api_views_code.encode('utf-8')).decode('ascii')

# Check if already added
stdin, stdout, stderr = ssh.exec_command(f"grep -c 'def openclaw_status' {SERVER_PATH}/views_index.py")
if '0' in stdout.read().decode():
    write_cmd = f'echo "{api_encoded}" | base64 -d >> {SERVER_PATH}/views_index.py'
    ssh.exec_command(write_cmd, timeout=10)
    print("  API views ADDED")
else:
    print("  API views already exist")

# Step 4: Add URLs
print("\n[4/6] Adding URLs...")

url_code = """    path('openclaw/api/status/', views_index.openclaw_status, name='openclaw_status'),
    path('openclaw/api/trigger-fix/', views_index.openclaw_trigger_fix, name='openclaw_trigger_fix'),
"""
...
"
"""
    ssh.exec_command(insert_cmd, timeout=10)
    print("  URLs ADDED")
else:
    print("  URLs already exist")

# Step 5: Add fix panel to base.html
print("\n[5/6] Adding fix panel to base.html...")

include_code = '{% include "includes/fix_panel.html" %}'
include_encoded = base64.b64encode(include_code.encode('utf-8')).decode('ascii')

# Backup
ssh.exec_command(f"cp {SERVER_PATH}/templates/base.html {SERVER_PATH}/templates/base.html.bak2", timeout=5)

add_to_base_cmd = f"""python3 -c "
import base64
...
stdin, stdout, stderr = ssh.exec_command(add_to_base_cmd, timeout=10)
...
print("\nYou should see:")
print("  - If error: Fix panel with Manual Fix button + Progress bar")
print("  - If working: Normal login page")
