...
try:
    ...
    # 1. 备份原 settings.py
    print("\n[1] 备份原 settings.py...")
    stdin, stdout, stderr = ssh.exec_command('cp /var/www/eims/settings.py /var/www/eims/settings.py.backup')
    time.sleep(1)
    print("✓ 已备份到 settings.py.backup")
    
    # 2. 更新数据库密码
    print("\n[2] 更新数据库密码...")
    
    update_script = '''import re
...
'''
    ...
    stdin, stdout, stderr = ssh.exec_command(f'/var/www/eims/venv/bin/python << "DBTEST"\n{test_script}\nDBTEST')
