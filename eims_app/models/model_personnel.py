...
class Personnel(BaseModel):
    ...
    # 注意：添加 db_constraint=False 避免跨数据库外键约束导致模型混淆
    project = models.ForeignKey('ProjectDetail', on_delete=models.CASCADE, null=True, blank=True, verbose_name='主要项目', help_text='选择当前主要归属项目（可选）', related_name='main_personnel', db_constraint=False)
    project_code = models.CharField(max_length=50, verbose_name='项目编号', db_index=True, blank=True, default='')
    
    # 额外项目字段（支持一人多项目，最多 5 个）
    project2 = models.ForeignKey('ProjectDetail', on_delete=models.CASCADE, null=True, blank=True, verbose_name='项目 2', help_text='选择第二个项目（可选）', related_name='personnel_project2', db_constraint=False)
    project_code2 = models.CharField(max_length=50, verbose_name='项目 2 编号', db_index=True, blank=True, default='')
    
    project3 = models.ForeignKey('ProjectDetail', on_delete=models.CASCADE, null=True, blank=True, verbose_name='项目 3', help_text='选择第三个项目（可选）', related_name='personnel_project3', db_constraint=False)
    project_code3 = models.CharField(max_length=50, verbose_name='项目 3 编号', db_index=True, blank=True, default='')
    
    project4 = models.ForeignKey('ProjectDetail', on_delete=models.CASCADE, null=True, blank=True, verbose_name='项目 4', help_text='选择第四个项目（可选）', related_name='personnel_project4', db_constraint=False)
    project_code4 = models.CharField(max_length=50, verbose_name='项目 4 编号', db_index=True, blank=True, default='')
    
    project5 = models.ForeignKey('ProjectDetail', on_delete=models.CASCADE, null=True, blank=True, verbose_name='项目 5', help_text='选择第五个项目（可选）', related_name='personnel_project5', db_constraint=False)
    ...
    # 系统字段（注意：is_deleted, create_time, update_time 已在 BaseModel 中定义，不需要重复）
