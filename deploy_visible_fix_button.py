#!/usr/bin/env python3
"""
立即部署可见的手动修复按钮和自动刷新功能
Deploy visible manual fix button and auto-refresh
"""
import paramiko
import os
import time
...
print("=" * 80)
print("🔧 部署可见的手动修复按钮")
print("Deploy Visible Manual Fix Button")
print("=" * 80)
...
try:
    ...
    middleware_count = int(stdout.read().decode().strip())
...
'''
        ...
        # 添加到settings.py
        stdin, stdout, stderr = ssh.exec_command(r"""sed -i "/MIDDLEWARE = \[/a\\    'utils.middleware_autorefresh.AutoRefreshMiddleware'," /var/www/eims/settings.py""", timeout=5)
...
'''
    ...
    # 检查是否已添加
    stdin, stdout, stderr = ssh.exec_command("grep -c 'views_openclaw_api' /var/www/eims/urls.py")
    url_count = int(stdout.read().decode().strip())
    
    if url_count == 0:
        # 添加导入
        stdin, stdout, stderr = ssh.exec_command("sed -i '/from eims_app.views_openclaw_fix import/a from eims_app.views_openclaw_api import trigger_fix, check_status' /var/www/eims/urls.py", timeout=5)
        
        # 添加路由
        stdin, stdout, stderr = ssh.exec_command(r"""sed -i '/path\(\"openclaw\/api\/trigger-fix\/\", views_openclaw_fix.trigger_fix,/i\    path(\"openclaw/api/trigger-fix/\", trigger_fix, name=\"trigger_fix\"),\n    path(\"openclaw/api/check-status/\", check_status, name=\"check_status\"),' /var/www/eims/urls.py""", timeout=5)
        print("  ✅ URL路由已添加")
    else:
        print("  ⚠️  URL路由已存在")
...
wsgi:application > /var/www/eims/logs/gunicorn.log 2>&1 &"""
    ...
    # 最终测试
    print("\n" + "=" * 80)
    print("✅ 部署完成！可见的手动修复按钮已启用")
    print("=" * 80)
    
    print("\n现在请刷新浏览器页面，您将看到:")
    print("  1. 🎯 屏幕中央显示大型修复面板")
    print("  2. ⚡ 明显的'立即手动修复'按钮")
    print("  3. 🔄 '刷新页面'按钮")
    print("  4. 📊 实时进度条显示修复状态")
    print("  5. 🎨 美观的半透明遮罩层")
    ...
    print("\n💡 使用说明:")
    print("  • 页面加载后会自动显示修复面板")
    print("  • 可以点击'立即手动修复'按钮")
    print("  • 也可以点击'刷新页面'按钮")
    print("  • 系统会自动尝试修复并显示进度")
