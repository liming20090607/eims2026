...
from ..models import Personnel, Employee, Department
...
from ..forms.form_personnel import PersonnelForm
...
def personnel_list(request):
    ...
    from ..models import Employee, Personnel
    ...
    # 2. 获取排序参数
    sort_field = request.GET.get('sort_field', 'employee_code')  # 默认按编号排序
    sort_order = request.GET.get('sort_order', 'asc')  # 默认升序
    
    # 3. 确定使用哪个数据库
    system = getattr(request, 'current_system', 'jiachengda')
    db_alias = system if system in ['dingce', 'shengchang', 'jiachengda'] else 'jiachengda'
    ...
    # 这样可以显示所有在项目/部门中的人员，以及未分配的纯Employee
    
    # 4.1 获取所有Employee
    if tenant_id:
        all_employees = Employee.objects.using(db_alias).filter(is_deleted=False, tenant_id=tenant_id).order_by('employee_code')
    else:
        all_employees = Employee.objects.using(db_alias).filter(is_deleted=False).order_by('employee_code')
    
    # 构建Employee字典，方便快速查找
    employee_dict = {emp.id: emp for emp in all_employees}
    
    # 4.2 获取当前租户的所有Personnel（人员去向表）
    if tenant_id:
        all_personnels = Personnel.objects.using(db_alias).filter(
            is_deleted=False,
            tenant_id=tenant_id
    ...
    context = {
        ...
        'active_personnel': 0,
        'use_employee_model': True,  # 统一使用Employee模型字段
        'all_projects': ProjectDetail.objects.using(db_alias).order_by('project_code'),
        'all_departments': Department.objects.using(db_alias).filter(is_deleted=False, status='active').order_by('department_code'),
    }
